...
const uint8_t vehicleLength[numRoadLanes] = {2*unitLength,1*unitLength,1*unitLength, 1*unitLength,1*unitLength};
...
void drawVehicles(){
	...
	for(uint8_t lane = 0; lane<numRoadLanes;lane++){
		int16_t currentlane = (roadOffset + lane) * trackWidth -5;
		int8_t size = vehicleLength[lane];
		...
		for (uint8_t ve = 0; ve<maxVehiclesPerLane;ve++){
			...
			//Moving Right
			if (vSpeed > 0 ){
				if (vex.pos < vex.prevPos && vex.prevPos-vex.pos >200 ){
					rectangle newR = {0,vex.pos,currentlane,currentlane+15};
					rectangle oldR = {vex.prevPos-size,screenWidth,currentlane,currentlane+15};
				...
				//Moving right
					...
					rectangle newR = {vex.prevPos,vex.pos,currentlane,currentlane+15};
					rectangle oldR = {vex.prevPos-size,vex.pos-size,currentlane,currentlane+15};
					fill_rectangle(newR,laneColours[lane]);
					fill_rectangle(oldR,BLACK);
					roadLanes[lane][ve].prevPos = vex.pos;
				}
				...
				if (vex.pos > vex.prevPos && vex.pos-vex.prevPos >200 ){
					rectangle newR = {vex.pos-size,screenWidth,currentlane,currentlane+15};
					rectangle oldR = {0,vex.prevPos-size,currentlane,currentlane+15};
					fill_rectangle(newR,HOT_PINK);
					fill_rectangle(oldR,BLACK);
					roadLanes[lane][ve].prevPos = vex.pos;
				}
				else if (vex.pos < vex.prevPos){
					rectangle newR = {vex.pos-size,vex.prevPos-size,currentlane,currentlane+15};
					rectangle oldR = {vex.pos,vex.prevPos,currentlane,currentlane+15};
					if(vex.pos<size){
						newR = (rectangle){0,0,currentlane,currentlane+15};
						oldR = (rectangle){vex.pos,vex.prevPos,currentlane,currentlane+15};
					}
					else{
						newR = (rectangle){vex.pos-size,vex.prevPos-size,currentlane,currentlane+15};
						oldR = (rectangle){vex.pos,vex.prevPos,currentlane,currentlane+15};
					}
					fill_rectangle(newR,laneColours[lane]);
					fill_rectangle(oldR,BLACK);
...
void collision(){
	if (roadOffset<=mainFrog.track && mainFrog.track<numTracks )
	for (uint8_t ve = 0; ve<maxVehiclesPerLane;ve++){
...
ISR(TIMER3_COMPA_vect)
{
	...
	char buffer2[4];
	sprintf(buffer2, "%03d", lives);
	display_string_xy("Lives:", 20, 0);
	display_string_xy(buffer2, 60, 0);
	fps = 0;

	char buffer3[4];
	sprintf(buffer3, "%03d", score);
	display_string_xy("Score:", 80, 0);
	display_string_xy(buffer3, 120, 0);
	fps = 0;
}
